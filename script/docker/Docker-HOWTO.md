...
依赖：docker compose v2，删除`name: adjust-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name adjust-maven-repo

docker run -it --rm --name adjust-maven \
    -v adjust-maven-repo:/root/.m2 \
